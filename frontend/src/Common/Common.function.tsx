...
export const ComponentWrapper = (scale: number, state: string, data: GenericTypeComponent['data']): React.ReactNode => {
    switch (data.type) {
        ...
        case "knobInteger": {
            ...
            const rotation = getValueByKey(state, data.component.knobProps?.rotation);
        ...
        case "number": {
            const maxLength = data.component.stringProps?.maxStringLength;
            
            if (!maxLength) {
                const ErrorComponent = getComponentById("Error") as React.ComponentType<GenericTypeComponent>;
                return <ErrorComponent scale={scale} state={state} data={data}>
                    Error {data.backend.key} - maxStringLength not defined
                </ErrorComponent>;
            }

            const digits = readDigits(state.toString(), maxLength);
            const StringNumber = getComponentById("StringNumber") as React.ComponentType<GenericTypeComponent>;
            
            const elements = Array.from({length: maxLength}, (_, digit) => (
                <StringNumber
                    key={digit}
                    scale={scale}
                    state={digits[digit]}
            ...
            return (
                <ComponentStringContainer
                    container_width={data.component.position.imgWidth * maxLength}
